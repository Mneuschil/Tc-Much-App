    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ padding: spacing.xl, alignItems: 'center' }}>
        ...
        <Pressable onPress={handleAvatarPick} style={styles.avatarWrap}>
          <Avatar imageUrl={user.avatarUrl} firstName={user.firstName} lastName={user.lastName} size="xl" />
          <View style={[styles.cameraIcon, { backgroundColor: colors.backgroundSecondary, borderRadius: radii.pill }]}>
        ...
        {!editing ? (
          <>
            ...
            {user.phone && (
              <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.xs }]}>{user.phone}</Text>
            )}
            ...
            <View style={{ marginTop: spacing.xl, width: '100%' }}>
              <Button title="Profil bearbeiten" onPress={startEditing} variant="secondary" fullWidth />
            </View>
          </>
        ) : (
          /* Edit Mode */
          <View style={{ width: '100%', marginTop: spacing.xl, gap: spacing.md }}>
            <View>
              <Text style={[typography.label, { color: colors.textPrimary, marginBottom: spacing.xs }]}>Vorname</Text>
              <Input value={firstName} onChangeText={setFirstName} placeholder="Vorname" />
            </View>
            <View>
              <Text style={[typography.label, { color: colors.textPrimary, marginBottom: spacing.xs }]}>Nachname</Text>
              <Input value={lastName} onChangeText={setLastName} placeholder="Nachname" />
            </View>
            <View>
              <Text style={[typography.label, { color: colors.textPrimary, marginBottom: spacing.xs }]}>Telefon</Text>
              <Input value={phone} onChangeText={setPhone} placeholder="Telefonnummer" />
            </View>
            <Button title="Speichern" onPress={handleSave} variant="primary" fullWidth loading={updateProfile.isPending} />
        ...
        {teams && teams.length > 0 && (
          <View style={{ width: '100%', marginTop: spacing.xxxl }}>
            <Text style={[typography.h3, { color: colors.textPrimary, marginBottom: spacing.md }]}>Meine Teams</Text>
            {teams.map((team: Team) => (
              <View
                key={team.id}
                style={[styles.teamCard, { backgroundColor: colors.backgroundSecondary, borderRadius: radii.md, padding: spacing.md, marginBottom: spacing.sm }]}
              >
                <Text style={[typography.bodyMedium, { color: colors.textPrimary }]}>{team.name}</Text>
                {team.league && (
                  <Text style={[typography.caption, { color: colors.textSecondary, marginTop: 2 }]}>{team.league}</Text>
                )}
              </View>
            ))}
          </View>
