...
export function MessageBubble({
  ...
  const metaColor = isOwn ? 'rgba(255,255,255,0.6)' : colors.textTertiary;
    ...
    <View style={[styles.row, isOwn ? styles.rowOwn : styles.rowOther]}>
      ...
      >
        ...
        <View style={styles.metaRow}>
          ...
          <View style={{ flex: 1 }} />
          <Text style={[typography.caption, { color: metaColor, fontSize: 10 }]}>
            {formatTimeAgo(message.createdAt)}
          </Text>
        </View>

        <Pressable
          onPress={() => onReply({ id: message.id, content: message.content })}
          hitSlop={10}
          style={[
            styles.replyBtn,
            { right: isOwn ? undefined : -28, left: isOwn ? -28 : undefined },
          ]}
        >
...
const styles = StyleSheet.create({
  row: { flexDirection: 'row', marginBottom: 6, alignItems: 'flex-end', gap: 8 },
  ...
  replyBtn: { position: 'absolute', bottom: 4 },
