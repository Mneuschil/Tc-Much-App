...
beforeAll(async () => {
  ...
  const otherClub = await prisma.club.create({ data: { name: 'Other Team Club', clubCode: 'TEAMOTH1' } });
  otherClubId = otherClub.id;

  const adminUser = await prisma.user.create({
    data: {
      email: 'teamadmin@test.de', passwordHash, firstName: 'Admin', lastName: 'Team', clubId,
  ...
  const boardUser = await prisma.user.create({
    data: {
      email: 'teamboard@test.de', passwordHash, firstName: 'Board', lastName: 'Team', clubId,
  ...
  const memberUser = await prisma.user.create({
    data: {
      email: 'teammember@test.de', passwordHash, firstName: 'Member', lastName: 'Team', clubId,
  ...
  const member2User = await prisma.user.create({
    data: {
      email: 'teammember2@test.de', passwordHash, firstName: 'Member2', lastName: 'Team', clubId,
      roles: { create: [{ role: 'MEMBER', clubId }] },
    },
  });
  ...
  const otherUser = await prisma.user.create({
    data: {
      email: 'teamother@test.de', passwordHash, firstName: 'Other', lastName: 'Team', clubId: otherClubId,
      roles: { create: [{ role: 'MEMBER', clubId: otherClubId }] },
    },
  });

  // Create a team in other club for multi-tenant test
  await prisma.team.create({ data: { name: 'Other Club Team', type: 'MATCH_TEAM', clubId: otherClubId } });
...
describe('GET /api/v1/teams', () => {
  ...
  beforeAll(async () => {
    const mt = await prisma.team.create({ data: { name: 'Herren Test', type: 'MATCH_TEAM', clubId } });
    matchTeamId = mt.id;
    const tg = await prisma.team.create({ data: { name: 'Training Test', type: 'TRAINING_GROUP', clubId } });
...
describe('GET /api/v1/teams/:teamId', () => {
  ...
  beforeAll(async () => {
    const team = await prisma.team.create({ data: { name: 'Detail Team', type: 'MATCH_TEAM', clubId } });
    teamId = team.id;
...
describe('PUT /api/v1/teams/:teamId', () => {
  ...
  beforeAll(async () => {
    const team = await prisma.team.create({ data: { name: 'Update Team', type: 'MATCH_TEAM', clubId } });
    teamId = team.id;
  });
...
describe('DELETE /api/v1/teams/:teamId', () => {
  ...
  it('rejects BOARD_MEMBER from deleting (AC-10)', async () => {
    const team = await prisma.team.create({ data: { name: 'No Delete', type: 'MATCH_TEAM', clubId } });
