...
router.get('/attendance/:eventId', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put('/attendance', validate(setAttendanceSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/overview', requireAnyRole([UserRole.TRAINER, UserRole.BOARD_MEMBER, UserRole.CLUB_ADMIN, UserRole.SYSTEM_ADMIN]), async (req: Request, res: Response, next: NextFunction) => {
  try {
