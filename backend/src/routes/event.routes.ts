...
router.post('/', requireBoard, validate(createEventSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const event = await eventService.createEvent(req.body, req.user!.clubId, req.user!.userId);

    const io = req.app.get('io');
    if (io) {
      io.to(`club:${req.user!.clubId}`).emit('event:created', event);
    }

    // Auto-push to team members for team events (fire-and-forget)
    if (req.body.teamId) {
      pushService.sendToTeam(req.body.teamId, {
        title: 'Neues Event',
        body: `"${event.title}" wurde erstellt. Bitte Verfuegbarkeit eintragen.`,
        data: { eventId: event.id },
      }, req.user!.userId).catch(() => { /* swallow */ });
    }
...
router.put('/:eventId', requireBoard, validate(updateEventSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const updated = await eventService.updateEvent(req.params.eventId as string, req.user!.clubId, req.body);

    const io = req.app.get('io');
    if (io) {
      io.to(`club:${req.user!.clubId}`).emit('event:updated', { eventId: req.params.eventId });
    }
    ...
    if (rsvpUserIds.length > 0) {
      pushService.sendToUsers(rsvpUserIds, {
...
router.delete('/:eventId', requireAdmin, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/:eventId/availability', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const availabilities = await availabilityService.getAvailabilityForEvent(req.params.eventId as string);
...
router.get('/:eventId/availability/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put('/:eventId/availability', validate(availabilitySchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    await availabilityService.checkTeamMembership(req.params.eventId as string, req.user!.userId, req.user!.clubId);
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(`club:${req.user!.clubId}`).emit('availability:updated', availability);
    }

    // Lineup integration: notify captain if player was in lineup and now unavailable
    lineupService.handleAvailabilityChange(
      req.params.eventId as string,
      req.user!.userId,
      req.body.status,
    ).catch(() => { /* swallow */ });
...
router.post('/:eventId/availability/remind', requireBoard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await availabilityService.sendReminder(req.params.eventId as string, req.user!.clubId);
    success(res, result);
  } catch (err) {
    next(err);
  }
...
router.post('/:eventId/lineup', requireBoard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const event = await eventService.getEventById(req.params.eventId as string, req.user!.clubId);
    if (!event) {
      error(res, 'Event nicht gefunden', 404, 'NOT_FOUND');
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(`club:${req.user!.clubId}`).emit('match:lineup', { eventId: req.params.eventId });
    }
...
router.put('/:eventId/lineup', requireBoard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const event = await eventService.getEventById(req.params.eventId as string, req.user!.clubId);
    if (!event) {
      error(res, 'Event nicht gefunden', 404, 'NOT_FOUND');
...
router.post('/:eventId/lineup/confirm', requireBoard, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/:eventId/attendance', requireAnyRole([UserRole.TRAINER, UserRole.BOARD_MEMBER, UserRole.CLUB_ADMIN, UserRole.SYSTEM_ADMIN]), async (req: Request, res: Response, next: NextFunction) => {
  try {
